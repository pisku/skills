...
The `description` field is the primary triggering mechanism for your skill.
...
1. **What it does**: Primary capabilities
2. **When to use it**: Specific triggers and contexts
3. **Key capabilities**: Distinctive features
...
**Good example** (Git Commit Helper):
```yaml
...
```json
{
  ...
  "query": "Example user query that should trigger this skill",
...
1. **Triggering tests**: Should trigger on obvious/paraphrased queries; should NOT trigger on unrelated queries
2. **Functional tests**: Valid outputs, API success, error handling, edge cases
3. **Performance comparison**: Baseline comparison (with vs without skill on tokens, tool calls, failed calls)
...
Provide a default with an escape hatch rather than presenting many options upfront.
...
- **"Solve, Don't Punt"**: When writing scripts for Skills, handle error conditions rather than failing and letting Claude figure it out.

  **Good example** (handle errors):
  ```python
...
- **Self-documenting constants**: Justify configuration parameters to avoid "voodoo constants"
...
- `bigquery_schema` and `create_issue` are the tool names
...
**Claude Code-specific field**: `allowed-tools` is available only in Claude Code CLI to restrict which tools the Skill can use. Format: `allowed-tools: ["Read", "Grep", "Glob"]`. This field does NOT apply in SDK or API contexts.
...
- [ ] Error handling is helpful with specific messages
...
### Testing
- [ ] At least 3 evaluations created before writing documentation
- [ ] Baseline established (performance without skill)
- [ ] Tested with all models you plan to use (Haiku, Sonnet, Opus)
- [ ] Triggering accuracy verified (should trigger/should not trigger)
- [ ] Functional tests pass (valid outputs, error handling, edge cases)
- [ ] Performance comparison shows improvement over baseline
